  ...
  ================================================== -->
		...
		<div class="container marketing">
			...
			<div class="row featurette">
				...
				<%-- <%
				enctype="mutilpart/form-date" 였을때에는 기존 방식으로 web parameter처리가 될 수 없다.
				파일 업로드 컴포넌트를 사용. 
 ...
 */
				...
				<c:otherwise>
				업로더 : <c:out value="${uploader}"/>			
				대상층 연령 :
				<c:forEach var="age" items="${ageArr }">
					<c:out value="${age}"/> 
				</c:forEach>
				<br>
				원본파일명 : <c:out value="${originalName}"/>
				업로드파일명 : <c:out value="${fileSystemName}"/>
